...
# DEBUG MODE: Set to True for troubleshooting card content duplication
# WARNING: This logs card content - turn OFF in production
DEBUG_CARD_CONTENT = os.environ.get('DEBUG_TRELLO_CARDS', 'false').lower() == 'true'
    ...
    async def get_openai_response(self, prompt: str, max_tokens: int = 500):
        ...
        try:
            async with httpx.AsyncClient(timeout=60.0) as client:
                print("Starting OpenAI HTTP request...")
...
async def generate_topic_driven_lesson(card_id: str, topic: str, level: str = "B1"):
    """
    Natural emergence algorithm - generates lesson from topic
    Topic drives vocabulary → vocabulary drives grammar → everything connects naturally
    """
    print(f"Generating natural lesson for topic: {topic} at level {level}")
    
    try:
        # Get card/board details for context
        card_details = await trello.get_card(card_id)
        board_id = card_details.get('idBoard')
        
        if not board_id:
            await trello.add_comment(card_id, "❌ Could not determine board ID")
            return
            
        # Step 1: Extract natural vocabulary from topic
        vocabulary_prompt = f"""
        Topic: "{topic}"
        Student level: {level}
        
        Generate 8 essential vocabulary words that:
        1. Naturally belong together in this topic
        2. Are words students would actually need in real conversation
        3. Mix of word types (nouns, verbs, adjectives) as appropriate
        4. Level-appropriate ({level})
        
        Return ONLY a comma-separated list of 8 words.
        Example: headache, fever, medicine, tired, cough, doctor, rest, sick
        """
        
        vocabulary_response = await ai.get_openai_response(vocabulary_prompt, max_tokens=100)
        vocabulary = [word.strip() for word in vocabulary_response.split(',')][:8]
        
        # Step 2: Discover what grammar naturally emerges
        grammar_prompt = f"""
        These vocabulary words emerged from the topic "{topic}":
        {', '.join(vocabulary)}
        
        What grammar structure would people NATURALLY use with these words?
        Don't force a grammar - discover what native speakers actually say.
        
        Consider:
        - How do people naturally talk about {topic}?
        - What patterns appear when using these words?
        - What's the most practical/useful structure?
        
        Return:
        1. Grammar pattern (e.g., "I'd like..." or "have/has + noun")
        2. Why this naturally fits
        3. Three real-world example sentences
        
        Be concise and practical.
        """
        
        grammar_response = await ai.get_openai_response(grammar_prompt, max_tokens=300)
        
        # Step 3: Generate complete lesson where everything connects
        lesson_prompt = f"""
        Create a natural, cohesive lesson where everything connects to: "{topic}"
        
        Core elements discovered:
        - Topic: {topic}
        - Natural vocabulary: {', '.join(vocabulary)}
        - Emerging grammar: {grammar_response}
        - Level: {level}
        
        Follow this teaching sequence for 1:1 online adult class:
        
        📝 Class title: [Creative title related to {topic}]
        
        🎯 Class objectives: [3 specific, measurable objectives]
        
        🔥 Warm-up: [Topic activation that feels like natural conversation]
        
        📚 Target vocabulary: {', '.join(vocabulary)}
        
        🎲 Vocabulary practice: [Activity where students use words naturally, not just memorize]
        
        📘 Grammar focus: [Explain the natural pattern that emerged - not as rules but as "how people say it"]
        
        ✏️ Guided practice: [Practice that mirrors real conversation about {topic}]
        
        🔗 Integration activity: [Vocabulary + grammar working together naturally]
        
        🗣️ Communicative activities:
        - Speaking: [Real situation where they'd discuss {topic}]
        - Role play: [Authentic scenario related to {topic}]
        
        ✅ Wrap-up: [Check they can now talk about {topic} naturally]
        
        🌟 Closing phrase: [Motivational, related to {topic}]
        
        IMPORTANT: Every activity should feel like natural communication about {topic}, 
        not grammar exercises. Students should feel they're learning to talk about {topic},
        not learning grammar rules.
        """
        
        lesson_content = await ai.get_openai_response(lesson_prompt, max_tokens=2500)
        
        # Create card in Trello with the natural lesson
        list_id = await get_or_create_list(board_id, "Natural Lessons")
        if list_id:
            card_name = f"{topic} - Natural Lesson ({level})"
            new_card_id = await create_variant_card(board_id, list_id, card_name, lesson_content)
            
            if new_card_id:
                await trello.add_comment(card_id, f"✅ Created natural lesson: {card_name}")
                if DEBUG_CARD_CONTENT:
                    print(f"NATURAL LESSON CONTENT: {lesson_content[:400]}...")
            else:
                await trello.add_comment(card_id, f"❌ Failed to create lesson card")
                
    except Exception as e:
        print(f"Error in natural lesson generation: {e}")
        import traceback
        traceback.print_exc()
        await trello.add_comment(card_id, f"❌ Error generating natural lesson: {str(e)}")
...
async def generate_framework_variants(framework_id: str, framework_data: dict, num_variants: int, card_id: str):
    ...
    try:
        ...
        # Get or create "VariantsV2" list
        variants_list_id = await get_or_create_list(board_id, "VariantsV2")
        if not variants_list_id:
            await trello.add_comment(card_id, "❌ Could not create VariantsV2 list")
            return
            
        await trello.add_comment(card_id, f"✅ Using variants list: {variants_list_id}")
        
        # Generate detailed variants with AI
...
"""
            ...
            # Get AI response
            print(f"About to make OpenAI request for variant {i+1}")
            try:
                ai_response = await ai.get_openai_response(prompt, max_tokens=3000)
                print(f"Got AI response for variant {i+1}: {len(ai_response) if ai_response else 0} chars")
                
                # DEBUG: Log content preview for troubleshooting duplication
                if DEBUG_CARD_CONTENT and ai_response:
                    print(f"VARIANT {i+1} CONTENT PREVIEW: {ai_response[:300]}...")
                    print(f"VARIANT {i+1} THEME/LEVEL: {theme}/{level}")
                    
            except Exception as openai_error:
                print(f"OpenAI API error for variant {i+1}: {openai_error}")
                import traceback
                traceback.print_exc()
                continue  # Skip this variant and try next one
        ...
        await trello.add_comment(card_id, f"✅ Completed generating {num_variants} variants!")
...
# AWS Lambda uses lambda_handler as the entry point
